...
fn merge_two_app_proofs_into_mega() {
    ...
    let (merged_proof, merged_vk) = bb::merge_mega(&proof_a.0, &vk_a.0, &proof_b.0, &vk_b.0).expect("merge");
...
fn merge_merge_proofs_into_mega() {
    ...
    // First-level merges
    let (m1_proof, m1_vk) = bb::merge_mega(&proof_a.0, &vk_a.0, &proof_b.0, &vk_b.0).expect("merge1");
    let (m2_proof, m2_vk) = bb::merge_mega(&proof_a.0, &vk_a.0, &proof_b.0, &vk_b.0).expect("merge2");

    // Merge the merge proofs
    let (merged2_proof, merged2_vk) = bb::merge_mega(&m1_proof.0, &m1_vk.0, &m2_proof.0, &m2_vk.0).expect("merge of merges");

    // Verify final Mega proof
    let ok = bb::verify_mega_honk(&merged2_proof.0, &merged2_vk.0).expect("verify merged of merges");
    assert!(ok);
}
